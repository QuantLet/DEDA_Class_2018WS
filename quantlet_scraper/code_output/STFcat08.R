...
# simHPP, simNHPP, Paretornd, simNHPPALP, BondZeroCouponHPP, simHPPALP
# ---------------------------------------------------------------------------- Quantlet: STFcat08
# ---------------------------------------------------------------------------- Description: STFcat08 plots the difference
# between zero-coupon CAT bond prices in the Burr claim amount case for a homogeneous and a non-homogeneous Poisson process
# governing the flow of losses. ---------------------------------------------------------------------------- Author: Awdesch
# Melzer 20130629 ----------------------------------------------------------------------------
...
BondZeroCouponHPP = function(Z, D, T, r, lambda, distr, params, Tmax, N) {
    # computes price of the zero-coupon CAT bond for the given claim amount distribution and the homogeneous Poisson process
    # governing the flow of losses --------------------------------------------------------------------- Usage: y =
    # BondZeroCouponHPP(Z,D,T,r,lambda,distr,params,Tmax,N)
    # --------------------------------------------------------------------- Input: Parameter: Z Definition: scalar, payment at
    # maturity Parameter: D Definition: n1 x 1 vector, threshold level Parameter: T Definition: n2 x 1 vector, time to expiry
    # Parameter: r Definition: scalar, continuously-compounded discount rate Parameter: lambda Definition: scalar, intensity of
    # the Poisson process Parameter: distrib Definition: string, claim size distribution Parameter: params Definition: n x 1
    # vector, parameters of the claim size distribution n = 1 (exponential) n = 2 (gamma, lognormal, Pareto, Weibull) n = 3
    # (Burr, mixofexps) Parameter: Tmax Definition: scalar, time horizon Parameter: N Definition: scalar, number of trajectories
...
simHPPALP = function(lambda, distrib, params, T, N) {
    ...
    # intensity of the Poisson process Parameter: distrib Definition: string, claim size distribution Parameter: params
    # Definition: n x 1 vector, parameters of the claim size distribution n = 1 (exponential) n = 2 (gamma, lognormal, Pareto,
    # Weibull) n = 3 (Burr, mixofexps) Parameter: T Definition: scalar, time horizon Parameter: N Definition: scalar, number of
    # trajectories --------------------------------------------------------------------- Output: Parameter: y Definition:
    # 2*max+2 x N x 2 array, generated proecss - max is the maximum number of jumps for all generated trajectories
...
distr = "Burr"
# distr = 'lognormal' params = c(18.3806,1.1052) #lognormal
...
wireframe(z ~ x * y, data = data, screen = list(z = 70, x = -60), drape = TRUE, colorkey = F, ticktype = "detailed", scales = list(arrows = FALSE, 
