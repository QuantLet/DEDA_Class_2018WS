...
simNHPP = function(lambda,parlambda,T,N){
    ...
    while(i&lt;=N){
        pom = JM[,i,1][JM[,i,1]
<t] !="Burr" #="" ##########################="" #[1:i,])="" #paretornd="" &&="" (distrib="Burr" (i="" (lambda="2){" (lambda)="1){" (lambda)!="1){" (length(n))!="1){" (length(params))!="1){" (length(t))!="1){" (missing(alpha)){="" (missing(lambda)){="" (missing(m)){="" (missing(n)){="" )="" ){="" 0="" 0,1="" 1="" 1,="" 1:m){="" 2="" 2,="" 2.")="" 3="" <="0" a="" alpha="1" alpha,="" an="" and="" array="" arrays="" aux="min(as.matrix(which(poisproc[,i,1]==T)))" be="" be:="" burr="" burr")="" chosen="" cpp="dim(poisproc)[2]" default="" distrib="" distribs="" distribution="" distribution,="" distribution.="" distributions,="" either="" else="" en="NROW(pom)" exponential="" exponential,="" for="" from="" gamma,="" i="1" if="" if((distrib="gamma" if(aux="" if(distrib="" if(lambda="" if(length(parlambda)!="2" if(n="1){" if(t="" in="" inversion="" lambda="" lambda!="2){" lambda,="" lambda.="" lambdat="(a+b*sin(2*pi*(pom+d))^2)/(a+b)" lognormal,="" losses="matrix(0,rpp,cpp)" m="1" m-by-n="" main="" maxen="max(maxEN,EN)" method.="" mixofexps="" mixofexps,="" must="" n="" n,="" numbers="" of="" or="" parameters="" params="" pareto="" paretornd="" parlambda="" poisproc="simNHPP(lambda,parlambda,T,N)" pom="pom[R&lt;lambdat]" positive="" program="" r="runif(NROW(pom))" random="" respectively.="" return(y)="" return(yy)="" returns="" rpp="dim(poisproc)[1]" scalar.")="" should="" simnhppalp="function(lambda,parlambda,distrib,params,T,N){" stop("simnhppalp:="" t="" the="" u="matrix(0,n,m)" u[,i]="lambda*(runif(n,0,1)^(-1/alpha)-1)" uses="" values="" vector.")="" weibull="" weibull,="" while(i<="N){" with="" x="" y="u" yy="yy[1:(2*maxEN+2),,]" yy[(2*en+1):rjm,i,2]="matrix(EN,nrow=rjm-2*EN,ncol=1)" yy[1:(2*en+1),i,1]="c(0,rep(pom,each=2))" yy[2:(2*en),i,2]="c(floor((1:(2*EN-1))/2))" {="" ||="" }="" }else{="" }}}="">
 2){
