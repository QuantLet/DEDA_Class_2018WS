...
# Simulate true coefficients (beta) with a change of L1-norm after i = n.cp
tmp1  = rep(1, 10)
tmp2  = rep(0, n.param - length(tmp1))
b1    = c(tmp1, tmp2)   # Coefficients for i &lt;= n.cp

tmp3  = seq(1, 0.1, -0.1)
tmp4  = rep(0, n.param - length(tmp3))
b2    = c(tmp3, tmp4)   # Coefficients for i &gt; n.cp
...
set.seed(seed2)
for (i in 1:n.sim){
  eps[[i]] = rALD(n.obs, mu = 0, sigma = 1, p = tau)
} 

# Compute all n.obs observations of Y
Y1    = list()
for (i in 1:n.sim){
  Y.tmp = numeric(0)
  for (j in 1:n.cp){
