# Heuristic 1: multi-input transactions are by the same address holder
# Heuristic 2: output addresses which have not appeared before are 
# shadow addresses and belong to the same input address holder
...
parsing = function(data, size, scenario, original){
  ...
  print(paste("Heuristic I:", round(print.out / length(uniqueAdd), 4), 
              "of all addresses matched"))
  ...
  for (i in 1:nrow(data)){
    if (data[i, 4] != 2 &amp; data[i, 5] != 2){
      if (sum(data[i, 4:5] %in% data[1:(i-1), ]) == 1){
        heuristic2[i, ] = c(data[i, 2], data[i, 4:5]
                             [which(!data[i, 4:5] %in% data[1:(i-1), ])])
      }
    }
  }
  heuristic2 = heuristic2[complete.cases(heuristic2), ]
  print(paste("Heuristic II:", round(nrow(heuristic2) / length(uniqueAdd), 4), 
              "of all addresses matched"))
  
  
  colnames(heuristic1) = colnames(heuristic2) = c("A", "B")

  # assign unique ID to each transaction that is copied to any transaction using the account
  heuristics    = rbind(heuristic1, heuristic2)
  heuristics    = unique(heuristics)
  heuristics$ID = 1:nrow(heuristics)
  
  print(paste("Heuristics I + II", round(nrow(heuristics) / length(uniqueAdd), 4), 
              "of all addresses matched"))
  
  for (i in 1:nrow(heuristics)){
    check = heuristics[i, 1:2]
    heuristics[apply(apply(heuristics, 2, function(x) x %in% check), 1,
                     function(x) any(x) == T), 3] = heuristics[i, 3]
  }
