...
#  search for better projection vector
  ...
  pmax = 1
  while (i
<n){ "directions"))="" (ind="" cind="rbind(2,1)" cind[1]="2" cind[which.max(inds)]="4" cind[which.min(inds)]="1" h)="" i="i+1" if="" ind="ppf(xp," inds="c(inds,ind)" p="p/sqrt(sum(p^2))" pind="cbind(1:length(inds),inds)" tit="toString(paste(i," xp="x%*%p">
 imax){
      imax = ind
