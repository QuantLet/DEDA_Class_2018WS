        ...
        convertMenuItem(
          menuItem("Step 4 - Start Simulation", tabName = "step4",
                   actionButton(inputId = "startsimul", 
                                label = HTML("Click to start simulation
<br/>
(will only change tabs c and d)"))
          ), tabName = "step4"),
        convertMenuItem(
  ...
  output$resetable_input &lt;- renderUI({
    ...
    shiny::div(id=letters[(times %% length(letters)) + 1],
               fluidRow(column(6, align = "center",
                               ...
                               textInput(inputId = "mu_y", label = HTML("μ
<sub>
 y
 <!--"), value = parameters[2]),
                               textInput(inputId = "sigma_y", label = HTML("&sigma;<sub-->
 y
 <!--"), parameters[3]),
                        ...
                        column(6, align = "center",
                               ...
                               textInput(inputId = "sigma_v", label = HTML("&sigma;<sub-->
 v
 <!--"), parameters[8]),
                               textInput(inputId = "rho_j", label = HTML("&rho;<sub-->
 j
 <!--"), parameters[9]),
                               textInput(inputId = "mu_v", label = HTML("&mu;<sub-->
 v
 <!--"), parameters[10]))))
  })
