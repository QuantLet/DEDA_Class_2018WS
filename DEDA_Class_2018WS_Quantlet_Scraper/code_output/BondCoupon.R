...
simNHPP = function(lambda,parlambda,T,N){
    ...
    while(i&lt;=N){
        pom = JM[,i,1][JM[,i,1]
<t] !="Burr" #="" #[1:i,])="" #paretornd="" &&="" (burr,="" (distrib="Burr" (exponential)="" (gamma,="" (i="" (lambda="2)" (lambda)="1){" (lambda)!="1){" (length(n))!="1){" (length(params))!="1){" (length(t))!="1){" (missing(alpha)){="" (missing(lambda)){="" (missing(m)){="" (missing(n)){="" (n="2" )="" ){="" -="" ---------------------------------------------------------------------="" 0="" 0,1="" 1="" 1,="" 1:m){="" 2="" 2*max+2="" 2,="" 2.")="" 3="" <="0" a="" aggregate="" all="" alpha="1" alpha,="" an="" and="" array="" array,="" arrays="" aux="min(as.matrix(which(poisproc[,i,1]==T)))" be="" be:="" burr="" burr")="" by="" chosen="" claim="" cpp="dim(poisproc)[2]" default="" definition:="" distrib="" distribs="" distribution="" distribution,="" distribution.="" distributions,="" driven="" either="" else="" en="NROW(pom)" exponential="" exponential,="" for="" from="" function="" function,="" gamma,="" generated="" generates="" horizon="" i="1" if="" if((distrib="gamma" if(aux="" if(distrib="" if(lambda="" if(length(parlambda)!="2" if(n="1){" if(t="" in="" input:="" intensity="" inversion="" is="" jumps="" lambda="" lambda!="2){" lambda,="" lambda.="" lambdat="(a+b*sin(2*pi*(pom+d))^2)/(a+b)" linear="" lognormal,="" loss="" losses="matrix(0,rpp,cpp)" m="1" m-by-n="" max="" maxen="max(maxEN,EN)" maximum="" method.="" mixofexps="" mixofexps)="" mixofexps,="" must="" n="" n,="" non-homogeneous="" number="" numbers="" of="" or="" otherwise)="" output:="" parameter:="" parameters="" params="" pareto="" pareto,="" paretornd="" parlambda="" poisproc="simNHPP(lambda,parlambda,T,N)" poisson="" pom="pom[R&lt;lambdat]" positive="" process="" process.="" r="runif(NROW(pom))" random="" respectively.="" return(y)="" return(yy)="" returns="" rpp="dim(poisproc)[1]" scalar,="" scalar.")="" should="" simnhppalp="function(lambda,parlambda,distrib,params,T,N){" sine="" size="" square="" stop("simnhppalp:="" string,="" t="" the="" time="" trajectories="" u="matrix(0,n,m)" u[,i]="lambda*(runif(n,0,1)^(-1/alpha)-1)" uses="" values="" vector,="" vector.")="" weibull="" weibull)="" weibull,="" while(i<="N){" with="" x="" y="" yy="yy[1:(2*maxEN+2),,]" yy[(2*en+1):rjm,i,2]="matrix(EN,nrow=rjm-2*EN,ncol=1)" yy[1:(2*en+1),i,1]="c(0,rep(pom,each=2))" yy[2:(2*en),i,2]="c(floor((1:(2*EN-1))/2))" {="" ||="" }="" }else{="" }}}="">
 2){
