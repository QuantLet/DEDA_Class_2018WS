...
<i) &&="" (mi="">
 1)) {
                if ((Su&gt;infl*Smat[mi+1,i])||(Sl
 <infl*smat[mi-1,i])) ((su="" if="" sl="F^2/Su" su="sqrt(F^3/Smat[mi-1,i])" {="" }="">
  infl*Smat[mi+1,i])||(Su
  <infl*s)) ((sl="" if="" su="infl*(S+Smat[mi+1,i])/2" {="" }="">
   infl*S) || (Sl
   <infl*smat[mi-1,i])) #="" ####="" #######="" #******="" ((su="" (j="" (j<="i)" (smat[j-1,i]!="0)" *************************="" 2="" :="" a="" bs="" c="IBTblackscholes(s0,F,r,sigma,i*dt,Call_Put_Flag)" call="" call_put_flag="1" compensation="" dc="C*infl-rho_u" else="" f="infl*S" find="" for="" from="" func="1" i="j-1," i^1="j" if="" in="" interpolation,="" llnode="mi" lnode="mi+1" model="" node="" nodes="" of="" parabola="" price="" rho_u="sum(ADmat[j:i,i]*(infl*Smat[j:i,i]-F))" s="Smat[j-1,i]" s_n^i,="" seq(lnode+1,i+1))="" sigma="IBTimpliedvola(s0,F,i*dt,func)" sl="infl*(S+Smat[mi-1,i])/2" smat[mi+1,i+1]="Su" smat[mi,i+1]="Sl" step="" stock="" su="(Smat[j-1,i+1]*dc-ADmat[j-1,i]*F*(F-Smat[j-1,i+1]))/(C*infl-rho_u-ADmat[j-1,i]*(F-Smat[j-1,i+1]))" tree="" upper="" {="" }="">
    infl*Smat[j,i]) || (Su&lt; infl*S)) {
