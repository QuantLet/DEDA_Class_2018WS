...
# n=100 set.seed(201502977) cos a=0.5 n=100 set.seed(201502077) cos a=0.5
...
for (k in 1:Nsim) {
    ...
    ########### non-orthogonal xtt = cbind(incvec, xt) least squares projection matrix of
    ########### intercept and xt proj.li.xt= xtt %*%(solve( t(xtt) %*% xtt))%*% t(xtt)
    
    # to make z1 (x1.th) and z2 (x2.th) orthogonal to xt x1.th = (diag(1,
    # nrow=samplen, ncol=samplen)- proj.li.xt) %*%x1 x2.th = (diag(1,
    # nrow=samplen, ncol=samplen)- proj.li.xt) %*% x2
